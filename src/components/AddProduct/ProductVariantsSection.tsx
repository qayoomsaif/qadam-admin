        ...
        {allVariants.map((variant) => (
          <div key={variant._id}>
            ...
            <div className="w-full flex gap-4 flex-wrap mt-2">
              {variantImages[variant._id]?.map((image, index) => (
                <div
                  key={index}
                  className="w-[8rem] h-[8rem] sm:w-[10rem] sm:h-[10rem] relative rounded-lg bg-gray-100 flex items-center justify-center"
                >
                  <CloseIcon
                    style={{
                      position: 'absolute',
