...
const Category = () => {
  ...
  useEffect(() => {
    if (selectedCategory && activeTab) {
      const activeSubCat = selectedCategory.subcategory.find(
        (subCat: Subcategory) => subCat._id === activeTab
      )
    ...
    const fetchProducts = async () => {
      ...
      try {
        const data = {
          ...
          page,
          itemPerPage: Constants.general.itemPerPage,
        }
        ...
        <div className="mt-20 mb-4 border-b overflow-x-auto">
          ...
          <div id="default-styled-tab-content mt-1">
            {selectedCategory.subcategory.map((subcategory: any) => (
              <div
                key={subcategory._id}
                id={`styled-${subcategory._id}`}
                role="tabpanel"
                aria-labelledby={`${subcategory._id}-tab`}
                className={`p-4 rounded-lg bg-gray-50 ${activeTab === subcategory._id ? '' : 'hidden'}`}
              >
                <div className="text-sm text-gray-500 dark:text-gray-400">
                  {subcategory.product_count === 0 ? (
                    <h6>No products available for {subcategory.name}</h6>
                  ) : (
                    <ProductCard
                      productData={productsData[activeTab]?.data ?? []}
                    />
                  )}
